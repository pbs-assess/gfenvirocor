...
Because we do not yet have appropriately aggregated information on current variability, we use average spawning season PDO and NPGO indices as possible proxies for these types of mechanisms. 
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-coef-violins-", scenario, "-100-draws-brms-",full_rdev_var_count,".png")))
```
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-timeseries-", scenario, "-100-draws-",full_rdev_var_count,".png")))
```
...
<!-- knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-timeseries-", scenario, "-100-draws-",full_rdev_var_count,"-no-outlier.png"))) -->
<!-- ``` -->

\clearpage

<!-- < fig 10 > -->
(ref:fig-rdev-cond) Predicted relationships between body condition in SDs from the mean of each time series for mature females (A, yellow) and mature males (B, turquoise) and recruitment deviations from the base model for `r rdev_years[1]` through `r rdev_years[2]`. The darker shading represents the uncertainty in the predicted second order orthogonal polynomial relationship based on the combined posteriors from 100 time-series regressions, one for each pair of recruitment and body condition samples. Lighter shading indicates a 95% credible interval from the full posterior prediction (i.e., including uncertainty on new “observations” of recruitment deviations). Vertical dot-lines represent the median recruitment deviation and the 95% quantiles of observed values in the 100 MCMC samples. Horizontal dot-lines represent the median and the 95% quantiles of scaled deviations in predicted annual body condition observed in 100 samples from the condition index. Dots representing earlier years are progressively more transparent

```{r rdev-cond, echo=FALSE, fig.align='center', fig.pos='h', out.width="6in", fig.cap="(ref:fig-rdev-cond)."}
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-condition-corr-timeseries-",scenario,"-start-", rdev_years[1], "-100-draws-bmrs-poly-lag.png")))
```

<!-- Option to add violin plots -->
<!-- Density violin plots (C) are as described in Figure \@ref(fig:rdev-violins) -->
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/cond-enviro-corr-coef-violins-", scenario, "-100-draws-brms-",full_cond_var_count,".png")))
```
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/cond-enviro-corr-timeseries-", scenario, "-100-draws-brms-",full_cond_var_count,".png")))
```

<!-- Include for relevant variables: -->
...
Recruitment deviations (**TODO**) in `r species` were found to be **TODO** (Figure \@ref(fig:rdev-violins-short)). 
...
# knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-timeseries-", scenario, "-100-draws-", short_rdev_var_count ,"-short.png")))
```
...
# knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/cond-enviro-corr-coef-violins-", scenario, "-100-draws-brms-",short_cond_var_count,"-just-poly.png")))
```

<!-- Add to caption for relevent variables -->
