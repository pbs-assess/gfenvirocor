...
All variables are available as monthly means from 1993 to 2019 at a resolution of $4\ \mathrm{km}^2$, with an extent covering Canada's Pacific Exclusive Economic Zone. 
...
A subset of these variables was selected based on an initial exploration of correlations with recruitment deviations (Figure \@ref(fig:ev-indices)). 
...
Upwelling and temperature have been found to have strong effects on cohort strength in `r species` [@hayman1980]. 
...
We used annual values for annual indices (e.g. BI), whereas for indices and environmental variables available at a monthly resolution, we took the average across a three month seasonal window corresponding to the timing of the hypothesized effect (e.g. summer = June to August). 
We also applied varies lags (1 or 2 years) to environmental variables and indices to represent the ecosystem state prior to egg incubation or female spawning body condition. 
All environmental variables were centered by their mean and scaled by their SD before analysis. 

Spawning occurs at depths `r spawn_min` m to `r spawn_max` m [@love2011] (Figure \@ref(fig:spawn-O2)), so we calculated the mean sea floor oxygen concentration, salinity, and temperature for all cells containing these depths for each month and then took the average of across the a three month window. 
...
The prey for larval Pacific flatfish such as `r species` includes copepods and zooplankton during the pelagic phase [@moser1993]. 
Therefore, we derived an index of overall abundance of various copepod size groupings using anomalies reported in Galbraith et al. [@galbraith2024].
We also used indices of euphausiid abundance generated from modelling work done by Evans et al. [@evans2023] using summer survey data. 
These euphausiid indices of abundance represent three regions--WCVI, QCS, and offshore. 
Given uncertainty in where `r species` larvae are spending their pelagic period, we considered total euphausiid biomass anomalies for each region separately.

<!-- < fig 1 > -->
(ref:fig-ev-indices) Sample of climate indices and BCCM physical-biogeochemical model variables (oxygen, salinity, temperature, phytoplankton) used in our analyses. All except ones for euphausiid are currently available in the PACea R package. BCCM variables are aggregated as means across all cells containing any of the depths occupied by `r species` of the relevant age class.
...
We first ran a linear and quadratic regression with the environmental variables and climate indices at various temporal periods (seasonal) (Figure \@ref(fig:lm-reg) & \@ref(fig:qm-reg)). 
For the quadratic model, we transformed the predictor variables to the second orthogonal degree of the polynomial, where the first component represents the linear slope portion, and the second component represents the curvature. 
Variables that showed an R^2 value of greater than 0.1 were selected for further analysis with the Bayesian model (Table \@ref(tab:var-table)).

<!-- < fig 5 > -->
```{r lm-reg, echo=FALSE, fig.align='center', fig.pos='h', out.width="6in", fig.cap="Linear model regressions with selected climate indices and environmental variables. Covariates selected here had an $R^2$ value greater than 0.1. Shaded area represents the 95% confidence interval."}
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-timeseries-Final11-lm-148sub.png")))
```

<!-- < fig 6 > -->
```{r qm-reg, echo=FALSE, fig.align='center', fig.pos='h', out.width="6in", fig.cap="Quadratic model regressions with selected climate indices and environmental variables. Covariates selected here had an $R^2$ value greater than 0.1. Shaded area represents the 95% confidence interval."}
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-timeseries-Final11-lm-quadratic-148sub.png")))
```

<!-- < table 1 > -->
```{r var-table}

env_table <- readRDS(here::here(paste0("stock-specific/",spp,"/figs/env_var_table.rds")))

knitr::kable(
  env_table,
  caption = "Summary of variables grouped by category, showing the maximum R^2 across linear (lm) and quadratic (qm) models and whether any member of the group was selected (R^2 > 0.1). The seasonal temporal resolution tested for each variable are grouped into their broader environmental variable categories in this table."
)

```

For each sampled time-series of our biological responses, we fit separate Bayesian hierarchical generalized linear mixed effects models (GLMMs) to assess the effect of each environmental variable on recruitment deviations. 
...
We assessed model convergence on a model fit to the median estimates for recruitment deviations with 4 chains each with 2000 iterations (1000 warmup).
All models achieved Rhat (scale-reduction factors) of $< 1.01$ and effective sample sizes of $> 800$ (or 200 per chain).

Our analyses that account for both the uncertainty in recruitment deviations and the environmental covariates suggest most covariates are only weakly correlated (Figure \@ref(fig:rdev-violins) & \@ref(fig:rdev-pred)).
Those covariates include a few climate indices such as the prior year's winter NPGO and the current year's spring ONI.
These indices are driven by upwelling and productivity, as well as temperature phases driven by oceanic currents. 
Thus, they may be indicators of the ecosystem prior to spawning and juvenile rearing events for dover sole. The physical environmental variables tested (e.g. O2, salinity, temperature) did not show a strong lienar relationship with recruitment deviations, but some variables (e.g seafloor temperature) appeared to show parabolic/u-shaped relationships, suggesting an optimal range for any environmental effects.
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-coef-violins-start-1979-Final11-100-draws-brms-29.png")))
```
...
knitr::include_graphics(here::here(paste0("stock-specific/",spp,"/figs/rdev-enviro-corr-timeseries-start-1979-Final11-100-draws-29.png")))
```
...
To assess possible impacts of environmental conditions on Dover Sole, we considered recruitment as a biological response.  We selected a set of climate and biotic indices that we hypothesized to be potentially important for egg and pelagic larval survival (Appendix F). These included average climate indices and environmental variables that accounted for the spawning period (`r spawning_months`) and depths (`r spawn_min` m - `r spawn_max` m) at the start of each recruitment year as well as in prior years as an index for the environment leading up to egg incubation, or female spawning body condition (e.g. Oceanic Niño Index (ONI), North Pacific Gyre Oscillation Index (NPGO), Current Bifurcation Index (BI), Pacific Decadal Oscillation (PDO), mean sea floor oxygen concentration, and mean sea floor temperature). Similarly, we used climate indices and environmental variables tha accounted for sea surface conditions during the full potential pelagic period (`r pelagic_months`) (e.g. ONI, BI, sea surface salinity, sea surface dissolved O2 concentration). For both the spawning and pelagic larval periods, we averaged environmental variables and climate indices over three-month seasonal windows to better capture short-term variability, rather than using longer multi-month averages that would smooth over important fluctuations. While many of these variables are correlated (Figure \@ref(fig:env-cor)), our approach was to run each variable separately in a univariate analysis; more consideration would need to be taken for a multivariate approach.  

## Bayesian time-series regressions

For recruitment, we used 100 MCMC samples of the recruitment deviations from the ensemble base model. 
We restricted this analysis to the period from `r rdev_years[1]` to `r rdev_years[2]` for which age data with the potential to inform recruitment estimates were available. 

For each recruitment time series sample, We tested the correlation between recruitment and a range of climate indices and environmental variables.
These variables were first filtered using simple linear and quadratic regressions to retain only those variables with an R^2 value over 0.1 with the median recruitment estimates. 
Then we used a Bayesian approach of a time-series regression to assess the strength of evidence for each relationship. 
Regressions were again conducted on the median estimates for each variable combination to confirm convergence of the Bayesian model and then repeated for each MCMC sample recruitment time series. 
We then combined the posteriors from all the regressions across samples to derive posterior distributions that accounted for uncertainty in both the stock assessment and the regression models. 
Full description of Bayesian time-series regression and the details of all the following data sources are provided in [cite appendix]. 
...
Recruitment deviations in `r species` showed low to moderate (0.1 < R2 < 0.3) correlations with a number of environmental variables in the initial linear and quadratic regression analysis (Figure \@ref(fig:lm-reg) & \@ref(fig:qm-reg)). 
However, these correlations were less apparent when we accounted for uncertainty in recruitment output from the assessment (Figure \@ref(fig:rdev-violins) & \@ref(fig:rdev-pred)). 
The variables that showed the strongest relationships in the initial linear and quadratic regressions were the current bifurcation index for the recruitment year, NPGO during the winter in the year prior to recruitment, and the ONI in the spring of the recruitment year. However, all of these relationships had R2 values less than 0.25. 
When the Bayesian time series analysis was used to better account for uncertainty in recruitment deviations, most of these effects were diminished.
Some climate indices (e.g. MEI, NPGO, BI, and ONI) may have an effect on recruitment deviations, however the environmental variables (non-climate indices) showed a zero to minimal effect on recruitment deviations. 
Some climate indices and environmental variables showed some curvature relationship (parabolic or u-shaped). 

Given the lack of any strong relationship between Dover Sole recruitment and the range of environmental and climate variables explored, we did not attempt to integrate environmental conditions into the stock assessment model.  
